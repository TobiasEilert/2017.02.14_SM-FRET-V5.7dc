...
function result = analyze_movie_tif(filename, thresh, half_region, cameraside, correct_min, correct_backgnd, map, alex, start_frame, stop_frame)
disp('Loading movie ...');
if stop_frame == 0 || stop_frame > ceil((99 + start_frame) / 1.5)
    stop_frame = ceil((99 + start_frame) / 1.5);
end
frame_number = start_frame:1:start_frame+99;
current_movie = 0;
total_frames = start_frame + 99;
breakpoint = start_frame + 99;
rerun = 0;
info = [];
green_start = 1;
while min(frame_number) < total_frames
    ...
    [movie, info, green_start, total_frames, current_movie, breakpoint] = load_movie_tif(filename, info, cameraside, alex, green_start, total_frames, frame_number, current_movie, rerun, breakpoint);

    % 2. reduce end frame of movie when using ALEX
    if stop_frame > total_frames || stop_frame*1.5 > movie.size
    ...
    % 15. extract intensities
        ...
        frame_number = max(frame_number) + 1:1:max(frame_number) + 100;
...
result.backgnd_trace = backgnd_trace_start;
if alex == 1
    result.time_trace_red = time_trace_red_start;
    result.backgnd_trace_red = backgnd_trace_red_start;
end
result.coordinates = molecules_found;
