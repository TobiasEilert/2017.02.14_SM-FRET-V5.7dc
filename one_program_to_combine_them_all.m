...
% 5th column in MW: gamma value
% 2th column in FRW: FRET value
%Z:\__Nicole Malkusch\TIRF
%load files for Molecule-wise analysis
[file path] = uigetfile('*_res.txt','Loading FRET Results','MultiSelect','on', 'Z:\Nicole Malkusch\');
...
[file path] = uigetfile('*_FRETonly_trace.txt','Loading FRET Results','MultiSelect','on', path);
file_temp = file(1,2:end);      %somehow MultiSelect messes up the order... --> dirty fix!
file = [file_temp, file(1,1)];
clear file_temp;
data_FRW= [];
...
nonzero_FRW_DIFF = find(NEWMOL_ROW(:,1));%nonzero_FRW_DIFF tells the zeile were a new molecule starts
...
k=1; % k counts zeilen in output_MW_dummy
    ...
    for i = 1:zeilen
        if thilodummy(i,6) > 0.25; %Here an arbitrary threshold is set between 0.25 and 1.5
            if thilodummy(i,6)< 1.5;
           output_MW_dummy(1,1) = k; %fortlaufende zeilennummer
           output_MW_dummy(1,2) = data_MW(i,1); % movie number
           output_MW_dummy(1,3) = data_MW(i,2); % molecule number
           output_MW_dummy(1,4) = data_MW(i,4); % FRET value per molecule
           output_MW_dummy(1,5) = data_MW(i,6); % gamma value per molecule
           k=k+1;
           l = nonzero_FRW_DIFF(i,1); %l counts zeilen in data_FRW 
          
           %b counts how many frames are in one molecule
           
           if i~=zeilen %if it is NOT the last molecule in the movie take the starting frame of the next molecule and substract the starting frame of the active molecule
              b=(nonzero_FRW_DIFF(i+1,1)-nonzero_FRW_DIFF(i,1));
          else
           b=size(data_FRW,1)-(nonzero_FRW_DIFF(i,1)-1);
           %number of frames for the last molecule is calculated: 
           %total number of frames - starting frame of the second to last
           %molecule -1
          end
           for j = 1:b
